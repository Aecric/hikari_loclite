...
- **速度**: 定位 odom (`/hikari_loc/odom`) **差分 `pose.position`** 得到。
  loclite 的 odom 只填 `pose`、`twist` 为空 (见 `src/system/loclite_node.cpp`), 故只能差分位置。
...
退出时打印三组分布 (min/mean/p50/p95/p99/p99.5/max):
- `speed(odom)` → 推荐 `max_speed_mps`
- `accel(imu)`  → 推荐 `max_accel_mps2`
- `accel(odom)` → **交叉参考**: 门控内部实际用 `Δv/dt`(对 ESKF 速度差分), 与此量同域。
...
- 若日后 C++ 给 odom 补了 `twist`(真实速度), 可省差分直接读, 本工具届时再加 `--use_twist`。
